...
var (
	studioName                     string
	studioButtonWidth              uint
	studioButtonHeight             uint
	studioImageReportPayloadLength uint
)
...
func init() {
	studioName = "Streamdeck Studio"
	studioButtonWidth = 144
	studioButtonHeight = 112
	studioImageReportPayloadLength = 1024
	streamdeck.RegisterDevicetype(
		studioName, // Name
		image.Point{X: int(studioButtonWidth), Y: int(studioButtonHeight)}, // Width/height of a button
		0xaa,                           // USB productID
		...
		"JPEG",                         // Image format
		studioImageReportPayloadLength, // Amount of image payload allowed per USB packet
		nil,
		GetImageHeaderStudio, // Function to get the comms image header
		GetImageAreaHeaderStudio,
	)
}
